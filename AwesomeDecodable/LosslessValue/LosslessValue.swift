...
@propertyWrapper
public struct LosslessValue<Element, Strategy: LosslessStringDecodingStrategy>: Decodable where Element == Strategy.Value {
    ...
    public init(from decoder: Decoder) throws {
        do {
            self.wrappedValue = try Element.init(from: decoder)
        } catch {
            ...
            self.wrappedValue = Element.init("\(result)") ?? Strategy.defaultValue
        }
    }
}

extension LosslessValue: Equatable where Element: Equatable {}
...
extension KeyedDecodingContainer {
    ...
    func decode<T, Strategy: LosslessStringDecodingStrategy>(_ type: LosslessValue<T, Strategy>.Type,
                   forKey key: KeyedDecodingContainer<K>.Key) throws -> LosslessValue<T, Strategy> {
        guard let result = try self.decodeIfPresent(type.self, forKey: key) else {
            return LosslessValue(wrappedValue: Strategy.defaultValue)
        }
